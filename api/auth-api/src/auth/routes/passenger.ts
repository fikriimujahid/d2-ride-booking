...
export async function registerPassengerAuthRoutes(app: FastifyInstance) {
  const aud = app.config.jwt.aud.passenger;

  app.post(
    '/passenger/auth/login',
    {
      schema: {
        tags: ['Passenger Auth'],
    ...
    const userResult = await app.db.query<{
      ...
      `select id, user_type, password_hash, is_active, locked_until, failed_login_count
       from users
       where user_type = 'PASSENGER' and (email = $1 or phone = $1)`,
      [body.identifier]
    );
    ...
    const accessToken = await app.jwt.signAccessToken({ userId: user.id, userType: 'PASSENGER', aud });
  ...
  app.post(
    ...
    const accessToken = await app.jwt.signAccessToken({ userId: rotated.userId, userType: 'PASSENGER', aud });
